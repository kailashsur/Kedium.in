...
const UPDATE_BLOG_MUTATION = gql`
  mutation UpdateBlog($blog_id: String!, $title: String, $thambnail: String, $description: String, $content: String, $tags: [String], $draft: Boolean) {
  updateBlog(blog_id: $blog_id, title: $title, thambnail: $thambnail, description: $description, content: $content, tags: $tags, draft: $draft) {
    title
    thambnail
    description
    tags
...
export default function UpdateCard({
  ...
  const [thambnailUploaded, setThambnailUploaded] = useState(blog.thambnail ? true : false);

  const [image, setImage] = useState<string | null>(blog.thambnail);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const [input, setInput] = useState<string>("");
  const [topics, setTopics] = useState<Array<string>>(blog.tags); // all the topisc array

  // Graphql Mutation
  const [updateBlog, { data, loading, error }] =
  ...
  async function handelUpload(e: React.MouseEvent<HTMLButtonElement>) {
  ...
  function handelOnChangeTitleInput(e: React.ChangeEvent<HTMLInputElement>) {
    setBlog({
      ...blog,
          ...
          <div className=" my-4 mt-6 flex flex-col gap-2 w-full">
            ...
            <div className=" flex gap-4 my-4 items-center">
              ...
              <button
                ...
                disabled={!blog?.title || blog?.title === "" ? true : false}
