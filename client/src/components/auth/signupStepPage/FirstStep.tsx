...
import { AuthState, enable, updateStep } from "@/store/slices/authSlice";
import React, { useEffect, useState } from "react";
import { addUser, UserState } from "@/store/slices/userSlice";
import { signIn } from "next-auth/react";

// defind the type of the props
interface Props {
  next: () => void;
  currentState: string;
}

const FirstStep: React.FC<Props> = ({ next, currentState }) => {
  const dispatch = useDispatch();
  const { step, state } = useSelector(
    (state: { Auth: AuthState }) => state.Auth,
  );
  const userData = useSelector((state: { User: UserState }) => state.User.data);

  function handelSigninRoute(e: React.MouseEvent<HTMLParagraphElement>) {
    e.preventDefault();
    currentState == "signup"
  ...
  function SignupWithEmail(e: React.MouseEvent<HTMLButtonElement>) {
    e.preventDefault();
    next();
  }
  ...
  if (step == 1) {
    return (
      <div className=" p-6 rounded-md flex flex-col justify-center ">
        <h2 className=" text-[28px] font-serif text-center">
          {currentState == "signup"
            ? "Join Medium."
            : currentState == "login"
              ? "Welcome back."
        ...
        <div className=" h-[312px] flex flex-col justify-between ">
          ...
          <div className=" w-64 mt-[50px] flex flex-col gap-3 text-base ">
            <button
              ...
              className=" w-full bg-white dark:bg-primary-background dark:text-primary-dart-background  p-2 rounded-md flex  justify-center items-center"
            >
              <svg
                width="24"
                height="24"
            ...
            <button
              className=" w-full text-base bg-white dark:bg-primary-background dark:text-primary-dart-background p-2 rounded-md flex justify-center items-center"
        ...
        <p className=" w-52 text-xs mt-14 text-center">
          ...
          {currentState == "signup"
            ...
            : currentState == "login"
              ? '"Sign up"'
              : ""}{" "}
          to agree to Medium’s Terms of Service and acknowledge that Medium’s
