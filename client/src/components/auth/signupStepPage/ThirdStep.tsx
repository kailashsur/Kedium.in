...
import { AuthState, updateStep } from "@/store/slices/authSlice";
...
export default function ThirdStep() {
    ...
    const state = useSelector((state : { Auth : AuthState})=>state.Auth.state)
...
console.log("User data == ", userData)
    function handelFullnameChange(e : React.ChangeEvent<HTMLInputElement> ) {
    ...
    async function handelSubmit(e : React.FormEvent<HTMLFormElement>) {
        ...
        if (userData.fullname != formDataObj.fullname) {
            ...
            try {
                const responce = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/u/updateuser`, 
                {
                    fullname : fullname
                },
                {
