...
const CheckboxButton = ({ value = "", selected = false, onChange }: { value: string, selected: boolean, onChange: (value: string, checked: boolean) => void }) => {
    ...
    return (
        <label className="inline-flex items-center space-x-2">
            <input
                ...
                value={value}
            />
            <span className={`px-4 bg-white py-2 rounded-full flex justify-center items-center text-center select-none ${checked ? ' text-green-700 border border-green-700' : 'bg-gray-100'}`}>
                {value}
                {checked ? (
